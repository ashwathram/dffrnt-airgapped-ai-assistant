...
export function UploadModal({
  ...
  const startUpload = async () => {
    ...
    // Hash all valid files concurrently
    const hashed = await Promise.all(
      entries.map(async (e) => {
        if (e.validationError) return e;
        const contentHash = await hashFile(e.file);
        return { ...e, contentHash };
      })
    );

    const checked = hashed.map((e) => {
      if (e.validationError) return e;
      // Name duplicate check
      const nameDup = existingDocuments.find(
        (d) => d.name.toLowerCase() === e.file.name.toLowerCase()
      );
      if (nameDup) return { ...e, duplicateKind: "name" as DuplicateKind, duplicateDocName: nameDup.name };
      // Content duplicate check via SHA-256 hash
      const contentDup = existingDocuments.find(
        (d) => d.contentHash && d.contentHash === e.contentHash
      );
      ...
      const newDocs: Document[] = results
        ...
        .map((e) => {
          ...
          return {
            ...
            uploadedAtTime: uploadDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }),
            uploadedAtMs: now,
            uploadedBy: "System",
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-8" style={{ background: "rgba(9,26,41,0.5)" }}>
        <div className="w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" style={{ background: "var(--card)" }}>
          ...
          <div className="flex flex-col gap-2 px-8 pb-8">
            <button onClick={() => resolveDuplicate("replace")} className="w-full flex items-center gap-2 px-4 py-4 rounded-xl text-sm font-medium text-white" style={{ background: "var(--primary)" }}>
              <RefreshCw size={14} /> Replace current version
            </button>
            <button onClick={() => resolveDuplicate("copy")} className="w-full flex items-center gap-2 px-4 py-4 rounded-xl text-sm font-medium border border-border hover:bg-muted transition-colors" style={{ color: "var(--foreground)" }}>
              <FilePlus size={14} /> Upload anyway (append copy)
            </button>
            <button onClick={() => resolveDuplicate("cancel")} className="w-full flex items-center gap-2 px-4 py-4 rounded-xl text-sm text-muted-foreground hover:text-foreground transition-colors">
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(9,26,41,0.5)", padding: "2rem" }}>
        <div
          ...
          style={{ background: "var(--card)", height: "calc(100vh - 4rem)" }}
        >
          {/* Fixed header */}
          <div className="px-8 pt-8 pb-4 flex items-center gap-4 flex-shrink-0 border-b border-border">
          ...
          <div className="px-8 pt-4 pb-2 flex-shrink-0">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide" style={{ letterSpacing: "0.06em" }}>
              Ingestion summary · {entries.length} file{entries.length !== 1 ? "s" : ""}
            </p>
          </div>

          {/* Scrollable file list — fills all remaining space */}
          <div className="flex-1 min-h-0 overflow-y-auto px-8 pb-4 flex flex-col gap-4" style={{ scrollbarWidth: "thin" }}>
            {entries.map((e, i) => {
              const ext = getExt(e.file.name);
              const color = TYPE_COLORS[ext] ?? "#5a6a78";
              const isIngested = e.result === "success" || e.result === "replaced" || e.result === "copy";
              const uploadedTs = new Date();
              const uploadedDate = uploadedTs.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" });
              const uploadedTime = uploadedTs.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });

              const MetaRow = ({ label, value }: { label: string; value: React.ReactNode }) => (
                <div className="flex items-start border-b border-border last:border-0">
                  <span className="w-24 flex-shrink-0 text-[10px] font-medium text-muted-foreground uppercase tracking-wide py-2 pl-4 pr-2">{label}</span>
                  <span className="flex-1 text-xs text-foreground py-2 pr-4 leading-relaxed">{value}</span>
                </div>
              );

              return (
                <div key={i} className="rounded-xl border border-border flex-shrink-0" style={{ background: isIngested ? "rgba(10,124,74,0.03)" : e.result === "error" ? "rgba(231,35,0,0.03)" : "rgba(0,0,0,0.02)" }}>
                  {/* Status header */}
                  <div className="flex items-center gap-4 px-4 py-2 border-b border-border">
                    <ResultIcon kind={e.result!} />
                    <div className="w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0" style={{ background: `${color}15`, color }}>
                      {TYPE_ICONS[ext] ?? <File size={12} />}
                    </div>
                    <p className="text-xs font-medium text-foreground flex-1 min-w-0">
                      {e.result === "success" ? "Ingested successfully" : e.result === "replaced" ? "Replaced existing" : e.result === "copy" ? "Uploaded as copy" : e.result === "cancelled" ? "Upload cancelled" : `Failed: ${e.validationError ?? "unknown error"}`}
                    </p>
                  </div>

                  {/* Vertical metadata rows */}
                  <MetaRow label="File name" value={<span className="break-all">{e.result === "copy" ? `${e.file.name} (copy)` : e.file.name}</span>} />
                  <MetaRow label="Size" value={formatSize(e.file.size)} />
                  <MetaRow label="Type" value={ext.toUpperCase() || "Unknown"} />
                  {isIngested && <MetaRow label="Uploaded" value={`${uploadedDate} · ${uploadedTime}`} />}
                  {e.fromFolder && <MetaRow label="Folder" value={<span className="flex items-center gap-1"><Folder size={10} />{e.fromFolder}</span>} />}
                  {isIngested && (
                    <MetaRow
                      label="Tags"
                      value={
                        e.selectedTagIds.length > 0
                          ? <div className="flex flex-wrap gap-1 py-0.5">
                              {e.selectedTagIds.map((id) => {
          ...
          {/* Fixed footer */}
          <div className="px-8 pb-8 pt-4 border-t border-border flex-shrink-0">
            <button onClick={onClose} className="w-full py-4 rounded-xl text-sm font-medium text-white" style={{ background: "var(--primary)" }}>Done</button>
          </div>
        </div>
      </div>
    );
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium border border-border hover:bg-muted transition-colors"
                  style={{ color: "var(--foreground)" }}
                >
