"""Generate the synthetic evaluation corpus into tests/eval/corpus/.

Every document is written with controlled ground truth: each fact checked by
cases.py appears verbatim here and nowhere else, so correctness scoring is
unambiguous. The corpus mirrors the three tasks the assistant serves (resume/HR
extraction, proposal & correspondence Q&A, internal knowledge base) and covers
all seven supported formats.

The generated files are committed; re-run only when changing the corpus:
    python -m tests.eval.corpus_gen
"""
...
# filename -> (comma-separated tags, description), used when (re-)ingesting.
UPLOADS = {
...
def main() -> None:
    ...
    for name, text in {**RESUME_TXT, **CORRESPONDENCE_TXT, **FAQ_TXT,
                       **PROPOSAL_MD, **POLICY_MD}.items():
