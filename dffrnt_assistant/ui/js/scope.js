...
let open = false;
let groupsCollapsed = {}; // tagType id -> true when its chip group is collapsed
...
export function renderScope() {
  ...
  const groups = state.tagTypes.map((tt) => {
    ...
    // "partial" (some but not all selected) gets a lighter tint so the header
    // always reflects selection state at a glance, not just full selection.
    const allOn = tags.every((t) => state.chatScope.includes(t.name));
    const anyOn = tags.some((t) => state.chatScope.includes(t.name));
    ...
    const headStyle = allOn ? `background:${tt.color};color:#fff;border-color:${tt.color}`
      : anyOn ? `background:${tt.color}26;color:${tt.color};border-color:${tt.color}`
      : `color:${tt.color};border-color:${tt.color}55`;
    const collapsed = !!groupsCollapsed[tt.id];
    // Faint tag-type tint + a colored left stripe carry the grouping, echoing
    // the reasoning-trace panel's accent-stripe pattern.
    return `
      <div class="scope-group" style="border-left-color:${tt.color};background:${tt.color}0d">
        <div class="scope-ghead" data-scope="collapse" data-type="${esc(tt.id)}" title="${collapsed ? 'Expand' : 'Collapse'} ${esc(tt.name)}">
          <button class="scope-gname${allOn ? ' on' : anyOn ? ' partial' : ''}" data-scope="type" data-type="${esc(tt.id)}" style="${headStyle}" title="Select all ${esc(tt.name)} tags">${esc(tt.name)}</button>
          <span class="scope-gtoggle" aria-hidden="true">${svg(collapsed ? 'chevronRight' : 'chevronDown')}</span>
        </div>
        ${collapsed ? '' : `<div class="scope-chips">${chips}</div>`}
      </div>`;
  }).join('');
  ...
  el.innerHTML = `
    ...
    ${open ? `<div class="scope-body">
      ...
      <div class="scope-groups">${groups}</div>
  ...
  el.querySelectorAll('[data-scope]').forEach((b) => {
    b.onclick = (ev) => {
      ...
      // The "select all" pill sits inside the clickable header; keep its click
      // from bubbling up and also collapsing the card.
      if (a === 'type') ev.stopPropagation();
      ...
      else if (a === 'collapse') groupsCollapsed[b.dataset.type] = !groupsCollapsed[b.dataset.type];
      renderScope();
    };
  });
}
