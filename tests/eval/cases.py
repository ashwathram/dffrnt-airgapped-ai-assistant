"""Evaluation queries grounded in the synthetic corpus (see corpus_gen.py).

Every fact in ``must_include`` appears verbatim in exactly one corpus document
(or a deliberately controlled set), so correctness scoring is unambiguous.

Case fields:
  query          the user question
  expected       filename(s) that should be retrieved
  task           resume-hr | proposal-corr | knowledge-base (reporting axis)
  checks         must_include — groups of alternatives; every group must match
                 (a group matches if ANY of its case-insensitive substrings does).
                 informational — record behaviour, exclude from correctness.
  tags           optional tag filter (exercises tag-scoped retrieval)
  expect_refusal correct behaviour is the canonical refusal sentence
"""
...
NEGATIVE_CASES = [
    ...
    {"query": "What was the Adecco Group's revenue in 2025?",
     "note": "was in an old corpus, absent now — stale-memory fabrication probe"},
...
# Cases with a retrievable expected document (retrieval metrics are undefined
# for the isolation probe, whose correct result is no matching document).
ANSWERABLE = list(CASES) + [c for c in TAG_CASES if c["expected"]]
